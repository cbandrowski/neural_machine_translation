...
OUT_CURVE       = os.path.join(OUT_DIR, "word_based_ben_seq2seq_attention_training_curve.png")
OUT_BLEU_HIST   = os.path.join(OUT_DIR, "word_based_ben_seq2seq_attention_bleu_histogram.png")
OUT_ATTENTION   = os.path.join(OUT_DIR, "word_based_ben_seq2seq_attention_heatmap.png")
...
def update_bleu_json(
    ...
    payload["models"]["attention"] = {
        "status": "complete",
        "script": "seq2seq_bengali.py",
        "architecture": "bidirectional LSTM encoder + additive attention + LSTM decoder",
        "split": SPLIT_MODE,
...
def plot_training_curve(history, save_path):
    ...
    plt.suptitle('Word-Based Attention Seq2Seq LSTM - Bengali -> English', fontsize=13,
                 fontweight='bold')
    plt.tight_layout()
...
def plot_bleu_histogram(per_sentence_bleus, save_path):
    ...
    ax.set_title('Word-Based Attention BLEU Distribution (Test Set)', fontweight='bold')
